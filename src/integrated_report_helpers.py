...
class IntegratedReportGenerator:
    ...
    def _generate_html_content(
        ...
        <head>
            ...
            <style>
                ...
                .ashoka-primary {{
                    color: #663399; /* Purple color for Ashoka University */
                }}
                .ashoka-logo {{
                    max-height: 60px;
                    margin-bottom: 1rem;
        ...
        <body class="bg-gray-50 min-h-screen">
            <div class="container mx-auto px-4 py-8">
                <header class="mb-10 text-center">
                    <div class="flex justify-center items-center mb-3">
                        <div class="px-4 py-2 bg-purple-100 rounded-lg flex items-center justify-center">
                            <span class="font-bold ashoka-primary">Koita Centre for Digital Health, Ashoka University</span>
                        </div>
                    </div>
            ...
            <footer class="bg-gray-100 py-6 mt-10">
                <div class="container mx-auto px-4 text-center text-gray-600 text-sm">
                    <p class="font-medium mb-2">KCDH-A, Trivedi School of Biosciences, Ashoka University</p>
                    <p>© 2025 Health Data Analysis</p>
                    <p class="mt-3 text-xs text-gray-500">This report is for informational purposes only. Please consult your healthcare provider for medical advice.</p>
                </div>
            </footer>
        </body>
        </html>
    ...
    def _generate_section_3_content(self, physio_data, meals_data, correlation_analysis):
        ...
        # Process meals data to get daily meals and their timestamps
        daily_meals = self._get_daily_meals(meals_data)
        
        # Get physiological metrics per day
        daily_metrics = self._get_daily_metrics(physio_data)
        
        # Generate the actual daily meal interactions table
        interactions_html = self._generate_daily_meal_interactions(daily_meals, daily_metrics)
        ...
        <section class="page-section">
            ...
            <div class="mb-6">
                <p class="text-gray-600">This section analyzes how your daily meals interact with your physiological metrics.</p>
            </div>
            
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                <div class="bg-white rounded-lg shadow p-6 lg:col-span-2">
                    <h3 class="text-lg font-medium mb-4">Daily Meal Interactions</h3>
            ...
            <div class="bg-blue-50 border-l-4 border-blue-400 p-4">
                <p class="text-blue-700">
                    <span class="font-medium">Insight:</span> 
                    The daily meal and physiological metrics analysis helps identify patterns between what you eat
                    and changes in your health metrics. Continue tracking both for personalized insights.
                </p>
            </div>
        </section>
        """
    ...
    def _get_daily_meals(self, meals_data):
        """
        Process meals data to get meals by day
        
        Returns: Dict mapping dates to list of meals
        """
        daily_meals = {}
        
        try:
            if meals_data is None or not hasattr(meals_data, 'shape'):
                return daily_meals
                
            # Check for date column
            if 'date' not in meals_data.columns:
                # Try to convert time column to date if it exists
                if 'time' in meals_data.columns:
                    try:
                        meals_data['date'] = pd.to_datetime(meals_data['time'], unit='ms')
                        meals_data['date'] = meals_data['date'].dt.date
                    except Exception as e:
                        logger.error(f"Error converting meal times to dates: {str(e)}")
                        return daily_meals
            
            # Make sure we have dish column
            dish_col = None
            for col in meals_data.columns:
                if col.lower() in ['dish', 'food', 'meal']:
                    dish_col = col
                    break
            ...
            for _, row in meals_data.iterrows():
                ...
                meal_date = row['date']
                if isinstance(meal_date, datetime) or isinstance(meal_date, pd.Timestamp):
                ...
                # Convert time to readable format if available
                meal_time_str = "Unknown time"
                if 'time' in row:
                    try:
                        meal_time = pd.to_datetime(row['time'], unit='ms')
                        meal_time_str = meal_time.strftime('%H:%M')
                    except:
                        pass
            ...
            daily_meals = {k: v for k, v in sorted(daily_meals.items())}
            
        except Exception as e:
            logger.error(f"Error processing daily meals: {str(e)}")
        
        return daily_meals
    
    def _get_daily_metrics(self, physio_data):
        """
        Process physio data to get metrics by day
        
        Returns: Dict mapping dates to dict of metrics
        """
        daily_metrics = {}
        
        try:
            # Check if data is in HealthDataCollection format or DataFrame
            if hasattr(physio_data, 'records'):
                # It's a HealthDataCollection
                for record in physio_data.records:
                    if record.quality_flag != 'good':
                        continue
                    
                    date_str = record.datetime.date().strftime('%Y-%m-%d')
                    
                    if date_str not in daily_metrics:
                        daily_metrics[date_str] = {}
                    
                    metric = record.metric_type
                    if metric not in daily_metrics[date_str]:
                        daily_metrics[date_str][metric] = []
                    
                    daily_metrics[date_str][metric].append(record.value_1)
                
                # Calculate averages for each day and metric
                for date_str, metrics in daily_metrics.items():
                    for metric, values in metrics.items():
                        # For steps, use sum instead of average
                        if metric == 'steps':
                            daily_metrics[date_str][metric] = sum(values)
                        else:
                            daily_metrics[date_str][metric] = sum(values) / len(values)
            elif hasattr(physio_data, 'shape'):
                # It's a DataFrame
                if 'date' in physio_data.columns and 'metric_type' in physio_data.columns and 'value' in physio_data.columns:
                    for _, row in physio_data.iterrows():
                        date_str = row['date']
                        ...
                        daily_metrics[date_str][metric].append(value)
    ...
    def _generate_daily_meal_interactions(self, daily_meals, daily_metrics):
        """
        Generate HTML table showing daily meals and corresponding metrics
        
        Args:
            daily_meals: Dict mapping dates to list of meals
            daily_metrics: Dict mapping dates to dict of metrics
        """
        if not daily_meals or not daily_metrics:
            return """
            <div class="bg-yellow-50 border-l-4 border-yellow-400 p-4">
                <p class="text-yellow-700">
                    Insufficient data to generate daily meal interactions.
                </p>
            </div>
            """
        
        # Find common dates that have both meals and metrics
        common_dates = sorted(set(daily_meals.keys()).intersection(set(daily_metrics.keys())))
        
        if not common_dates:
            return """
            <div class="bg-yellow-50 border-l-4 border-yellow-400 p-4">
                <p class="text-yellow-700">
                    No matching dates found between meals and physiological metrics.
                </p>
            </div>
            """
        
        # Take the most recent dates (up to 5)
        display_dates = common_dates[-5:]
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Other Metrics</th>
                </tr>
            </thead>
        ...
        for date_str in display_dates:
            ...
            meals_list = daily_meals[date_str]
            meals_html = "<ul class='space-y-1'>"
            for meal in meals_list:
                meals_html += f"<li><span class='font-medium'>{meal['time']}:</span> {meal['dish']}</li>"
            meals_html += "</ul>"
            
            # Format metrics for this day
            metrics = daily_metrics[date_str]
            
            # Heart rate
            hr_value = "N/A"
            for metric_key in ['hr', 'heart_rate']:
                if metric_key in metrics:
                    hr_value = f"{metrics[metric_key]:.0f} bpm"
                    break
            
            # Steps
            steps_value = "N/A"
            if 'steps' in metrics:
                steps_value = f"{metrics['steps']:.0f}"
            
            # Sleep
            sleep_value = "N/A"
            if 'sleep' in metrics:
                sleep_value = f"{metrics['sleep']:.1f} hrs"
            
            # Other metrics
            other_metrics_html = "<ul class='space-y-1'>"
            for metric, value in metrics.items():
                if metric in ['hr', 'heart_rate', 'steps', 'sleep']:
                    continue
                ...
                other_metrics_html += f"<li><span class='font-medium'>{metric}:</span> {display_value}</li>"
            
            other_metrics_html += "</ul>"
            if other_metrics_html == "<ul class='space-y-1'></ul>":
                other_metrics_html = "N/A"
            
            # Add row to table
            table_html += f"""
            <tr>
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{display_date}</td>
                <td class="px-6 py-4 text-sm text-gray-500">{meals_html}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{hr_value}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{steps_value}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{sleep_value}</td>
                <td class="px-6 py-4 text-sm text-gray-500">{other_metrics_html}</td>
            </tr>
            """
